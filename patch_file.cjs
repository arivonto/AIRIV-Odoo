...
const searchString = `  async loadMenus() {`;
const replaceEnd = `  async jsonRpc(`;

let start = text.indexOf(searchString);
let end = text.indexOf(replaceEnd);

if (start !== -1 && end !== -1) {
  let newLoadMenus = `  async loadMenus() {
