...
const targetLoadMenus = `  async loadMenus() {
    ...
    const endpoint = '/api/web/menu/load_menus?t=' + Date.now();
    ...
    try {
      ...
      try {
        ...
        throw new Error('Server returned HTML instead of JSON (Status ' + response.status + ' from ' + response.url + '): ' + responseText.substring(0, 100));
      }
...
const newLoadMenus = `  async loadMenus() {
    ...
    try {
      ...
      // Convert search_read results into the dictionary structure expected by the app
      const resultDict = {
        root: { id: 'root', children: [] as number[] }
      } as any;
...
if (content.includes('async loadMenus() {')) {
  // Let's replace by doing regex or string matching
  // It might be easier to use string slicing
  const startIndex = content.indexOf('  async loadMenus() {');
  const nextMethodIndex = content.indexOf('  async jsonRpc', startIndex);
  if (startIndex !== -1 && nextMethodIndex !== -1) {
    content = content.substring(0, startIndex) + newLoadMenus + '\n\n' + content.substring(nextMethodIndex);
  }
}
