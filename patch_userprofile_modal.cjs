...
// Replace the right-column Identity Verification Card entirely with nothing
code = code.replace(
  /\s*\{\/\* Identity Verification Card \*\/\}\s*<div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">[\s\S]*?Scan ID Now'\}\n               <\/button>\n             <\/div>\n          <\/div>/g,
  ''
);

// Add the button to the header banner
code = code.replace(
  /<div className="mb-1">\s*<button onClick=\{\(\) => setEditBioOpen\(true\)\}/g,
  `<div className="mb-1 flex items-center gap-3">
                  ...
                  <button onClick={() => setEditBioOpen(true)}`
);

// Redefine submitIdVerification mapping based on new schema
code = code.replace(
  /const payload = \{[\s\S]*?image_1920: base64Clean \|\| undefined\n      \};/g,
  `const payload = {
        vat: ocrData.nik,
        name: ocrData.name,
...
// Delete the old camera logic
code = code.replace(/const startCamera = async \(\) => \{[\s\S]*?const closeScanner = \(\) => \{/g, `const closeScanner = () => {`);

// Rewrite the Modal component for the scanner
code = code.replace(
  /\{\/\* Identity Scanner Modal \*\/\}\s*\{showIdScanner && \([\s\S]*?\{\/\* Edit Bio Modal \*\/\}/g,
