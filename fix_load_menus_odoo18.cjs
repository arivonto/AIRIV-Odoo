...
if (targetStart !== -1 && targetEnd !== -1) {
    ...
    try {
      // Fetch ALL menus, removing 'app_id' for Odoo 18 compatibility
      const menusData = await this.executeKw('ir.ui.menu', 'search_read', [[]], {
          fields: ["id", "name", "parent_id", "child_id", "action", "sequence", "web_icon", "web_icon_data"]
      });
      ...
      if (Array.isArray(menusData)) {
        ...
        menusData.forEach((m: any) => {
          ...
          resultDict[m.id] = {
             ...
             name: m.name, 
             actionID: actionID, 
             // Without app_id, we just use the id itself or null, 
             // but we will assign the root appID recursively in a pass later.
             appID: m.id, 
             children: [], 
             xmlid: '', 
...
`;
    ...
    console.log("Patched loadMenus for Odoo 18 compatibility.");
} else {
